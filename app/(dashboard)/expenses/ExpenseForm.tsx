...
import { createFuelLog } from '@/lib/actions/fuel.actions'
...
export default function ExpenseForm({ vehicles }: { vehicles: Vehicle[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!data.vehicleId || isNaN(data.liters) || isNaN(data.cost)) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-medium text-neutral-700 mb-1">Volume (Liters)</label>
          <input 
            name="liters" 
            type="number" 
            step="0.1" 
